...
test("renders compact poster badges with white text and native score palette", () => {
  ...
  assert.match(stylesheet, /\.better-scene-card__badge-bar\s*\{[\s\S]*top:\s*0\.5rem;/);
  assert.match(stylesheet, /\.scene-card\.better-scene-card \.card-controls\s*\{[\s\S]*left:\s*auto;[\s\S]*right:\s*0\.5rem;/);
